...
function hasCompletedToday(habit) {
    if (habit.__optimistic_marked)
        return true;
    ...
    evaluate(state, event) {
        ...
        if (targetHabit && event?.event_type === 'mark_done') {
            ...
            if (targetHabit.last_completed_at) {
                const daysSinceLastCompletion = Math.floor((new Date().getTime() - new Date(targetHabit.last_completed_at).getTime()) / (1000 * 60 * 60 * 24));
